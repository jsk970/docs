...
```
                            2).   将“临时表”作为查询数据来源表，生成rownum
```
select e.* , rownum
from(select * from employees order by salary desc) e --临时表
```
                            3).   通过判断rownum保留前5行数据（薪资最高的前5名）
